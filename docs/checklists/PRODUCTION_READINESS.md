...
- [ ] VPS `media_data` has encrypted off-host backup, restore/recreate evidence, capacity alerts,
      scanning/retention policy, and reviewed orphan cleanup. If external media is enabled, its S3
      migration, credentials, access, lifecycle, and rollback are accepted separately.
