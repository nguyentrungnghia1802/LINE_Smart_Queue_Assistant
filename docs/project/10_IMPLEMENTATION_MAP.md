...
Last verified on 2026-08-27 after the GHCR release-flow audit. The baseline rechecks
...
| `deploy/scripts`                                                        | Windows PowerShell 12-character manual GHCR API/Web build-push with full revision metadata; backup-gated VPS wrapper and rehearsal | `07`, `08`, ADR-042, ADR-044                   |
| `deploy/backup`                                                         | VPS backup/verify/list/restore, registry-independent tag deploy, automatic metadata rollback, and rehearsal                        | `07`, `08`, ADR-041, ADR-042, ADR-043, ADR-044 |
| `scripts/release`                                                       | Automatic validated-main GHCR workflow contract validation                                                                         | `07`, `08`, ADR-043, ADR-044                   |
| `.github/workflows/ci.yml`, `.github/workflows/deploy.yml`              | PR/main CI gates; automatic validated-main GHCR publication, approval, and backup-gated CD                                         | `07`, `08`, ADR-041, ADR-042, ADR-043, ADR-044 |
...
| Frontend build  | `VITE_API_URL`, `VITE_APP_NAME`, `LINE_LOGIN_LIFF_ID`, `VITE_LIFF_ENDPOINT_PATH`, `VITE_LIFF_DEFAULT_BOOKING_PATH`, `VITE_PAYMENT_MODE`, `VITE_PAYMENT_REDIRECT_BASE_URL` | Public build-time data                                   |
...
The current image repositories are the lowercase public GHCR packages
`ghcr.io/nguyentrungnghia1802/line-smart-queue-api` and
`ghcr.io/nguyentrungnghia1802/line-smart-queue-web`. Automatic publication uses the protected
workflow's `GITHUB_TOKEN` with job-scoped `packages: write`; manual publication uses the same
repository names and a pre-authenticated local Docker CLI, preserving the 12-character Git-derived
tag. The server keeps exact `LINE_QUEUE_*_IMAGE` refs and the backup/rollback tooling accepts
registry-qualified refs. Optional `LINE_QUEUE_*_LEGACY_REPOSITORY` keys are transition-only aliases
for resolving the previous Docker Hub release during the rollback window.
