...
CREATE TABLE auth_sessions (
  ...
  session_kind TEXT NOT NULL CHECK (session_kind IN ('business', 'customer')),
...
CREATE INDEX idx_auth_sessions_family ON auth_sessions(family_id, created_at);
CREATE INDEX idx_auth_sessions_expiry
  ON auth_sessions(LEAST(idle_expires_at, absolute_expires_at)) WHERE revoked_at IS NULL;
...
CREATE TABLE organization_applications (
  ...
  business_type TEXT NOT NULL CHECK (business_type IN ('restaurant','salon','clinic','retail','public_service','other')),
  ...
  expected_monthly_customers INT NOT NULL CHECK (expected_monthly_customers BETWEEN 1 AND 10000000),
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT organization_applications_postal_code_format CHECK (postal_code ~ '^[0-9]{3}-?[0-9]{4}$'),
  CONSTRAINT organization_applications_review_state CHECK (
...
CREATE UNIQUE INDEX uq_organization_applications_pending_email
  ON organization_applications (LOWER(work_email)) WHERE status = 'pending';
...
CREATE INDEX idx_organization_applications_organization
  ON organization_applications (organization_id) WHERE organization_id IS NOT NULL;
