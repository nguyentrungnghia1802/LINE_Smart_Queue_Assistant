 ...
 *   • 1 manager user : alice@queue-lab.test  (password: Demo@1234)
 *   • 1 staff user   : bob@queue-lab.test    (password: Demo@1234)
 */
export async function seed001Organization(client: Client): Promise<void> {
  ...
  await client.query(
    `INSERT INTO organization_members (organization_id, user_id, role, is_active) VALUES
       ($1, $2, 'manager', TRUE),
       ($1, $3, 'staff', TRUE)
  ...
  console.info('[seed] 001 — organization + manager + staff created');
}
