...
/**
 * Migration 000001 — Full schema creation
 *
 * This is the single consolidated migration derived from
 * db/schema/reset_line_queue_schema.sql (the canonical schema reference).
 *
 * It creates the entire database schema from scratch:
 *   - Utility functions & triggers
 *   - ENUM types
 *   - Identity / tenant tables
 *   - Catalog / queue / order tables
 *   - Support / automation tables
 *   - Indexes
 *
 * To reset the database for local development, run:
...
export async function up(pgm: MigrationBuilder): Promise<void> {
  ...
  // ── ENUM types ───────────────────────────────────────────────────────────────
  pgm.sql(`
    DO $$ BEGIN CREATE TYPE user_role AS ENUM ('customer','staff','manager','admin','super_admin'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    DO $$ BEGIN CREATE TYPE org_member_role AS ENUM ('owner','manager','staff'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    DO $$ BEGIN CREATE TYPE product_type AS ENUM ('product','service'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    DO $$ BEGIN CREATE TYPE queue_status AS ENUM ('closed','open','paused','archived'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    DO $$ BEGIN CREATE TYPE queue_type AS ENUM ('walk_in','appointment','priority','disaster'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    DO $$ BEGIN CREATE TYPE queue_entry_status AS ENUM ('waiting','called','serving','served','skipped','cancelled','no_show'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    ...
    DO $$ BEGIN CREATE TYPE notification_type AS ENUM ('queue_joined','queue_near_turn','queue_called','queue_skipped','queue_cancelled','queue_served','queue_no_show','payment_required','payment_received'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    DO $$ BEGIN CREATE TYPE notification_channel AS ENUM ('line_push','email','sms','in_app'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    DO $$ BEGIN CREATE TYPE notification_status AS ENUM ('pending','processing','sent','delivered','failed','cancelled'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    DO $$ BEGIN CREATE TYPE penalty_type AS ENUM ('no_show','late_arrival','excessive_cancel','manual'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    DO $$ BEGIN CREATE TYPE audit_actor_type AS ENUM ('user','system','line_webhook','scheduler'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
  `);
  ...
  pgm.sql(`
    CREATE TABLE IF NOT EXISTS organizations (
      ...
      CONSTRAINT organizations_public_qr_token_format CHECK (public_qr_token ~ '^[A-Za-z0-9_-]{8,128}$')
    );
  ...
  pgm.sql(`
    ...
    COMMENT ON COLUMN queues.auto_no_show_minutes IS 'Grace period in minutes before a called ticket is auto-marked no_show. NULL = use app/global default.';
    CREATE TRIGGER trg_queues_updated_at BEFORE UPDATE ON queues FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  `);

  // Create orders before queue_entries so the FK can be added
  pgm.sql(`
  ...
  pgm.sql(`
    ...
    CREATE TRIGGER trg_queue_entries_updated_at BEFORE UPDATE ON queue_entries FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  `);

  // Add the cross-FK between orders and queue_entries
  pgm.sql(`
    ALTER TABLE orders
      ADD CONSTRAINT orders_queue_entry_id_fkey
  ...
  pgm.sql(`
    ...
    CREATE INDEX IF NOT EXISTS idx_om_org_role_active ON organization_members(organization_id, role) WHERE is_active = TRUE;

    CREATE INDEX IF NOT EXISTS idx_la_line_user_id ON line_accounts(line_user_id);
    CREATE INDEX IF NOT EXISTS idx_la_user_id ON line_accounts(user_id);

    CREATE INDEX IF NOT EXISTS idx_products_org_active ON products(organization_id, product_type, is_active);
    CREATE INDEX IF NOT EXISTS idx_products_org_name ON products(organization_id, name);

    CREATE INDEX IF NOT EXISTS idx_queues_org_active ON queues(organization_id, status) WHERE is_active = TRUE;
    ...
    CREATE INDEX IF NOT EXISTS idx_orders_customer_user_id ON orders(customer_user_id) WHERE customer_user_id IS NOT NULL;
    CREATE INDEX IF NOT EXISTS idx_orders_queue_entry_id ON orders(queue_entry_id) WHERE queue_entry_id IS NOT NULL;
    CREATE INDEX IF NOT EXISTS idx_orders_completed_revenue ON orders(organization_id, created_at DESC)
      WHERE status = 'completed' AND payment_status = 'paid';

    CREATE INDEX IF NOT EXISTS idx_order_items_order_id ON order_items(order_id);
    CREATE INDEX IF NOT EXISTS idx_order_items_product_id ON order_items(product_id);
    CREATE INDEX IF NOT EXISTS idx_order_items_order_covering ON order_items(order_id)
      INCLUDE (product_name, quantity, subtotal, service_time_minutes);

    CREATE INDEX IF NOT EXISTS idx_qe_queue_status_ticket ON queue_entries(queue_id, status, priority DESC, ticket_number ASC);
    CREATE INDEX IF NOT EXISTS idx_qe_queue_waiting ON queue_entries(queue_id, priority DESC, ticket_number ASC)
      WHERE status = 'waiting';
    CREATE INDEX IF NOT EXISTS idx_qe_queue_active ON queue_entries(queue_id, status, created_at)
      WHERE status IN ('waiting', 'called', 'serving');
    CREATE INDEX IF NOT EXISTS idx_qe_user_active ON queue_entries(user_id, created_at DESC)
      WHERE user_id IS NOT NULL AND status IN ('waiting', 'called', 'serving');
    CREATE INDEX IF NOT EXISTS idx_qe_order_id ON queue_entries(order_id) WHERE order_id IS NOT NULL;
    CREATE INDEX IF NOT EXISTS idx_qe_called_timeout ON queue_entries(queue_id, called_at)
      WHERE status = 'called';

    CREATE INDEX IF NOT EXISTS idx_notif_pending ON notifications(created_at)
      WHERE status = 'pending';
    CREATE INDEX IF NOT EXISTS idx_notif_retry_due ON notifications(next_retry_at)
      WHERE status = 'failed' AND next_retry_at IS NOT NULL;
    CREATE INDEX IF NOT EXISTS idx_notif_entry_type ON notifications(queue_entry_id, type, created_at DESC)
    ...
    CREATE INDEX IF NOT EXISTS idx_penalty_user_recent ON penalty_records(user_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_penalty_org_user_recent ON penalty_records(organization_id, user_id, created_at DESC)
      WHERE organization_id IS NOT NULL;

    CREATE INDEX IF NOT EXISTS idx_qh_org_recent ON queue_histories(organization_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_qh_queue_recent ON queue_histories(queue_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_qh_entry ON queue_histories(queue_entry_id) WHERE queue_entry_id IS NOT NULL;

    CREATE INDEX IF NOT EXISTS idx_audit_actor ON audit_logs(actor_id, created_at DESC) WHERE actor_id IS NOT NULL;
    CREATE INDEX IF NOT EXISTS idx_audit_org ON audit_logs(organization_id, created_at DESC) WHERE organization_id IS NOT NULL;
